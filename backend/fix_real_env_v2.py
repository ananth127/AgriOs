...
def fix_real_db_v2():
    ...
    engine = create_engine(real_db_url, isolation_level="AUTOCOMMIT")
    
    with engine.connect() as conn:
        print("Applying schema changes individually (AUTOCOMMIT mode)...")
        ...
        try:
            conn.execute(text("ALTER TABLE farm_assets ADD COLUMN iot_settings JSONB;"))
