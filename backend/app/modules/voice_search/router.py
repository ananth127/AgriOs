...
async def voice_search(request: VoiceQueryRequest, current_user: User = Depends(get_current_user)):
    ...
    """
    print(f"[ROUTER] Received audio_data length: {len(request.audio_data) if request.audio_data else 0} chars")
    
    try:
        if request.audio_data and len(request.audio_data) > 100:
            audio_bytes = base64.b64decode(request.audio_data)
            print(f"[ROUTER] Decoded audio: {len(audio_bytes)} bytes")
        else:
            print("[ROUTER] No valid audio data, using mock")
            audio_bytes = b"mock_audio_data"
    except Exception as e:
        print(f"[ROUTER] Base64 decode error: {e}")
        audio_bytes = b"mock_audio_data"
