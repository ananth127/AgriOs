...
class DiagnosisService:
    ...
    def _vertex_ai_diagnosis(self, image_url: str, crop: str) -> models.DiagnosisLog:
        """
        Use AI (LiteLLM) for real image analysis.
        Implements intelligent model fallback on quota exceeded.
        """
        print(f"🔬 Analyzing image with LiteLLM: {image_url}")
        ...
        try:
             ...
             raise ImportError("LiteLLM service not found")
             
        # Model fallback chain (ordered by quota likelihood)
        models_to_try = [
            "gemini/gemini-1.5-flash",       # Primary: Fast and efficient
            "gemini/gemini-pro-vision",      # Fallback
        ...
        for model_name in models_to_try:
            try:
                ...
                # For litellm with images, we need to pass image url or base64
        ...
        # All models failed
        print(f"❌ All Vision models exhausted. Error: {last_error}")
        raise last_error
