...
def create_animal(db: Session, animal: schemas.AnimalCreate):
    ...
    # First check: If it exists, return it (Optimization for Re-Seeding/Idempotency)
    existing_animal = db.query(models.Animal).filter(models.Animal.tag_id == animal.tag_id).first()
    if existing_animal:
        # If it's a demo animal, log and return existing to prevent crash
        if animal.tag_id.startswith("COW-DEMO") or animal.tag_id.startswith("GOAT-DEMO"):
            print(f"🔄 Idempotency: Returning existing demo animal: {animal.tag_id}")
    ...
    try:
        ...
        db.rollback()
        # Race condition handling: Check if it was created by another request concurrently
        print(f"⚠️ IntegrityError caught for {animal.tag_id}. Looking up existing record...")
        existing_concurrent = db.query(models.Animal).filter(models.Animal.tag_id == animal.tag_id).first()
        if existing_concurrent:
            return existing_concurrent
        else:
            # If still fails and not found, attempt to unique-ify tag if not demo
            if not (animal.tag_id.startswith("COW-DEMO") or animal.tag_id.startswith("GOAT-DEMO")):
                 new_tag = f"{animal.tag_id}-{int(datetime.utcnow().timestamp())}"
                 print(f"⚠️ Retrying with new tag: {new_tag}")
                 db_animal.tag_id = new_tag
                 db.add(db_animal)
                 db.commit()
                 db.refresh(db_animal)
            else:
                 # Should theoretically not happen if duplicate exists, as lookup above should find it.
                 # But if it does, re-raise to be safe.
                 raise
    ...
    if not db_animal.qr_code:
        ...
        try:
           ...
           db.rollback() 
           # Ignore QR update errors if any
