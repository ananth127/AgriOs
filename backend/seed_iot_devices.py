"""
Seed IoT devices for Smart Monitor demonstration
This script adds the new columns (status, last_telemetry) and seeds demo data
"""
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if 'last_telemetry' not in columns:
                ...
                conn.execute(text("ALTER TABLE iot_devices ADD COLUMN last_telemetry JSON DEFAULT '{}'"))
                conn.commit()
        ...
        db.query(IoTDevice).filter(IoTDevice.hardware_id.like('DEMO-%')).delete()
        db.commit()
...
if __name__ == "__main__":
    print("Seeding IoT Devices for Smart Monitor...")
    seed_iot_devices()
