...
if not exist "venv" (
    ...
    python -m venv venv
    echo Installing Backend requirements in venv...
...
:: 2. Start Backend in a new window (with venv activated)
echo Starting Backend Server...
start "Agri-OS Backend" cmd /k "cd backend && title Backend (FastAPI) && call venv\Scripts\activate.bat && uvicorn main:app --host 0.0.0.0 --port 8000 --reload"
