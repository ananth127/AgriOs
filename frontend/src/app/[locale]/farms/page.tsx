        ...
        api.farms.list()
            .then((data: any) => {
                if (Array.isArray(data)) {
                    ...
                    // Auto-select logic: REMOVED to prioritize User Location (Red Marker)
                    // Users can manually select a farm to focus on it.
                    if (processedFarms.length > 0 && selectedFarmId !== undefined) {
                        // Only validate existing selection
                        const exists = processedFarms.find(f => f.id === selectedFarmId);
                        if (!exists) setSelectedFarmId(undefined); // Reset to "View All" if selected farm disappears
                    }
                } else {
