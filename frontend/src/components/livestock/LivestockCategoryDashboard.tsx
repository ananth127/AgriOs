...
import { ArrowLeft, Plus, Ruler, QrCode } from 'lucide-react';
...
export const LivestockCategoryDashboard: React.FC<CategoryDashboardProps> = ({ category, animals, onBack, onRegister }) => {
    const [logModalAnimal, setLogModalAnimal] = useState<any>(null);
    ...
    const stats = useMemo(() => {
        ...
        categoryAnimals.forEach(a => {
            // Simple Age Logic: < 1 year is young (Mock logic as we compare dates)
            ...
            // Production sum (mock for now, need field in animal object or separate aggregation)
        });
    ...
    return (
        <div className="space-y-6">
                    ...
                    <table className="w-full text-left text-sm text-slate-300">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {categoryAnimals.map(animal => (
                                <tr key={animal.id} className="hover:bg-white/5 group">
                                    <td className="p-4">
                                    ...
                                    <td className="p-4 text-right flex justify-end gap-2">
                                        <button
                                            onClick={() => setLogModalAnimal(animal)}
                                            className="p-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 rounded-lg text-xs font-medium"
                                        >
                                            + Log
                                        </button>
                                        <button className="p-2 hover:bg-white/10 rounded-lg text-slate-400 hover:text-white transition-colors group/qr relative">
                                            <QrCode className="w-5 h-5" />
                                            {/* Hover QR Preview */}
                                            <div className="absolute right-full top-1/2 -translate-y-1/2 mr-2 w-32 h-32 bg-white p-2 rounded-lg hidden group-hover/qr:block shadow-xl z-50">
                                                <QRCode
                                                    value={`agrios://livestock/${animal.id}`}
                                                    size={100}
            ...
            {logModalAnimal && (
                <LogProductionModal
                    isOpen={!!logModalAnimal}
                    onClose={() => setLogModalAnimal(null)}
                    animal={logModalAnimal}
                    onSuccess={() => {
                        // Ideally trigger a refresh or show toast, but for now just close
                        console.log("Production logged");
                        setLogModalAnimal(null); // Close modal on success
                    }}
                />
            )}
        </div>
