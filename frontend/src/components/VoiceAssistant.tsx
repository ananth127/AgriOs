        ...
        <>
            ...
            {isOpen && (
                <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <Card className="w-full max-w-md bg-slate-900 border-green-500/30 shadow-2xl shadow-green-900/20 relative overflow-hidden">
                        ...
                        <div className="relative z-10 flex flex-col items-center text-center space-y-6 py-8">
                            ...
                            <div className="space-y-2 min-h-[80px]">
                                {status === 'listening' && <p className="text-xl font-light text-slate-300">Listening...</p>}
                            ...
                            {response && (
                                <div className="bg-slate-800/50 p-4 rounded-xl w-full text-left border border-white/5 animate-in fade-in slide-in-from-bottom-4">
                                    <div className="flex items-start gap-3">
                                        <div className="mt-1 p-2 bg-green-500 rounded-lg text-slate-900">
                                    ...
                                    {response.intent === 'market_price' && (
                                        <div className="mt-4 p-3 bg-green-900/20 border border-green-500/20 rounded-lg flex justify-between items-center">
                                            <span className="text-sm text-green-300">Market Action</span>
                                            <button className="text-xs bg-green-600 hover:bg-green-500 px-3 py-1.5 rounded transition-colors" onClick={() => window.location.href = '/marketplace'}>
                                                View Market
                                            </button>
                                        </div>
                                    )}
                            ...
                            {status !== 'listening' && status !== 'processing' && (
                                <button onClick={startListening} className="text-sm text-slate-500 hover:text-white underline">
                                    Try Again
                                </button>
                            )}
                        </div>
